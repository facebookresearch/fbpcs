...
class BoltJob(DataClassJsonMixin, Generic[T, U]):
    ...
    def __post_init__(self) -> None:
        if self.stage_timeout_override and self.stage_timeout_override < 0:
            ...
            logger.warning(
                f"The configured timeout override of {self.stage_timeout_override} is greater than the maximum allowed value of {DEFAULT_CONTAINER_TIMEOUT_IN_SEC} seconds. Setting it to the max."
            )
