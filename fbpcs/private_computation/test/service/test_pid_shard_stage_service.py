...
class TestPIDShardStageService(IsolatedAsyncioTestCase):
    ...
    @data_provider(
        lambda: (
            itertools.product(
                ...
                [1, 2],
                ["CoXbp7BOEvAN9L1CB2DAORHHr3hB7wE7tpxMYm07tc0=", None],
            )
        )
    )
    @to_sync
    async def test_pid_shard_stage_service(
        ...
        test_hmac_key: Optional[str],
    ) -> None:
        pc_instance = self.create_sample_pc_instance(
            pc_role, test_num_containers, test_hmac_key
        )
    ...
    def get_args_expect(
        ...
        if hmac_key:
            args += " --hmac_base64_key=CoXbp7BOEvAN9L1CB2DAORHHr3hB7wE7tpxMYm07tc0="
        return [args]
