...
class AnonymizationDataPrepStageService(PrivateComputationStageService):
    """
    StageService for running the A&A Data Preparation stage for PD.
    ...
    async def run_async(
        ...
        self._logger.info("Running A&A data prep")
    ...
    async def _start_containers(
        ...
        - Store any required arguments, such as DP noise path, on the
            PrivateComputationInstance. Likely on the PA product config.
        ...
        self._logger.info(
            "Starting A&A data prep containers (well, not actually - I'm a skeleton)"
        )
        return [
            ContainerInstance(
                "dummy_container_dne",
    ...
    def _dummy_get_status(
        ...
        self._logger.info(
            "Fetching A&A data prep status (well, not actually - I'm a skeleton)"
        )
