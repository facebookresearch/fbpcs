...
# Usage: Check if all status are complete for Lift stages: id_match, compute and aggregate
set -e
...
function check_status_complete() {
    $docker_command get "$1" --config="$DOCKER_CLOUD_CONFIG_FILE" || exit 1
...
until check_status_complete $"$LIFT_PUBLIHSER_NAME" && check_status_complete $"$LIFT_PARTNER_NAME"
