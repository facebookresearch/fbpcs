...
void idSwapMultiKey(
  ...
  while (getline(spineIdFile, spineRow)) {
    ...
    for (int i = 0; i < idColumnIndices.size() && i < max_id_column_cnt; i++) {
      if (numIds > i) {
        row_ids.push_back(cols.at(i + 1));
      } else { // if number of ids is smaller, hence missing,
  ...
  while (getline(dataFile, line)) {
    ...
    std::vector<std::string> row_ids;
    for (int i = 0; i < idColumnIndices.size() && i < max_id_column_cnt; i++) {
      row_ids.push_back(rowVec.at(idColumnIndices.at(i)));
    }
    auto concat_id = std::accumulate(
        row_ids.begin(),
        row_ids.end(),
