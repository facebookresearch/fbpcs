...
void runShard(const std::string &inputFilename,
              const std::string &outputFilenames,
              const std::string &outputBasePath, int32_t fileStartIndex,
              int32_t numOutputFiles, int32_t logEveryN) {
  if (!outputFilenames.empty()) {
    ...
    RoundRobinBasedSharder sharder{inputFilename, outputBasePath, startIndex,
                                   endIndex, logEveryN};
    sharder.shard();
  } else {
...
void runShardPid(const std::string &inputFilename,
                 const std::string &outputFilenames,
                 const std::string &outputBasePath, int32_t fileStartIndex,
                 int32_t numOutputFiles, int32_t logEveryN,
  ...
  if (!outputFilenames.empty()) {
    std::vector<std::string> outputFilepaths;
    folly::split(',', outputFilenames, outputFilepaths);
    HashBasedSharder sharder{inputFilename, outputFilepaths, logEveryN,
                             hmacBase64Key};
    sharder.shard();
  } else if (!outputBasePath.empty() && numOutputFiles > 0) {
    std::size_t startIndex = fileStartIndex;
    std::size_t endIndex = startIndex + numOutputFiles;
    HashBasedSharder sharder{inputFilename, outputBasePath, startIndex,
                             endIndex,      logEveryN,      hmacBase64Key};
    sharder.shard();
  } else {
    XLOG(FATAL) << "Error: specify --output_filenames or --output_base_path, "
                   "--file_start_index, and --num_output_files";
