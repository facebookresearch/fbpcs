...
DEFINE_string(output_base_path, "",
              "Local or s3 base path where output files are written to");
DEFINE_int32(file_start_index, 0,
...
int main(int argc, char **argv) {
  ...
  if (!FLAGS_output_filenames.empty()) {
    ...
    RoundRobinBasedSharder sharder{FLAGS_input_filename, outputFilepaths,
                                   FLAGS_log_every_n};
    sharder.shard();
    ...
    RoundRobinBasedSharder sharder{FLAGS_input_filename, FLAGS_output_base_path,
                                   startIndex, endIndex, FLAGS_log_every_n};
    sharder.shard();
  } else {
