...
def main() -> None:
    ...
    try:
        ...
        if ip_address and server_hostname:
            ...
            write_routing_script_path = os.path.join(
                home_dir, "package/write_routing.sh"
            )
            # sudo permission to run this script is built into image in D41862758
            # with specified user group.
            os.system(
                f"sudo {write_routing_script_path} {ip_address} {server_hostname}"
            )
